...
class StateManager:
    def __init__(self, load_prior_state: bool = True):
        ...
        self.add_listener("model_changed", self.save_to_storage)  # Save state when model is updated
        self.add_listener("simulation_completed", self.save_to_storage)  # Save state when simulation is completed
        self.add_listener("fit_completed", self.save_to_storage)  # Save state when simulation is completed
        self.add_listener("simulation_deleted", self.save_to_storage)
    ...
    def load_storage_into_state(self):
        ...
        orphans = []
        self.from_json(app.storage.user["state-data"])  # Load SimData from user storage if available
        ...
        for sim in self.simulations.values():
            try:
                sim.find_and_link_model(self.models)  # TODO: handle case where model is not found (seems unlikely?)
            except ValueError:
                orphans.append(sim)
        for o in orphans:
    ...
    def _emit_collection_events(
        self, before: dict[str, set[uuid.UUID]], after: dict[str, set[uuid.UUID]], notify_listeners: bool
    ) -> None:
    ...
    def _reconcile_active_ids(self, reason: str = "") -> dict[str, tuple[uuid.UUID | None, uuid.UUID | None]]:
        ...
        if not expt_is_valid:
            ...
            if candidate is None and self._active_model_id is not None:
                candidate = self._latest_matching_id(self.expt_datas, lambda d: d.model_id == self._active_model_id)
            if candidate is None:
                candidate = self._latest_id(self.expt_datas)
            _set_active("_active_expt_data_id", candidate)

        if (
            self._active_raw_data_id is None
            and self._active_expt_data_id in self.expt_datas
            and self._active_expt_data_id is not None
        ):
        ...
        if not fit_is_valid:
            ...
            if candidate is None and self._active_expt_data_id is not None:
                candidate = self._latest_matching_id(self.fits, lambda f: f.expt_data_id == self._active_expt_data_id)
            if candidate is None and self._active_model_id is not None:
                candidate = self._latest_matching_id(self.fits, lambda f: f.model_id == self._active_model_id)
            if candidate is None:
        ...
        if not sim_is_valid:
            ...
            if self._active_model_id is not None:
                candidate = self._latest_matching_id(self.simulations, lambda s: s.model_id == self._active_model_id)
            if candidate is None:
        ...
        if not mcmc_is_valid:
            ...
            if candidate is None and self._active_expt_data_id is not None:
                candidate = self._latest_matching_id(self.mcmcs, lambda m: m.expt_data_id == self._active_expt_data_id)
            if candidate is None and self._active_model_id is not None:
                candidate = self._latest_matching_id(self.mcmcs, lambda m: m.model_id == self._active_model_id)
            if candidate is None:
    ...
    def _finalize_active_context(
        self, reason: str, emit_events: bool = True
    ) -> dict[str, tuple[uuid.UUID | None, uuid.UUID | None]]:
        changes = self._reconcile_active_ids(reason=reason)
        if emit_events:
            self._emit_active_context_events(changes)
        return changes

    def reconcile_active_context(
        self, reason: str = "manual", emit_events: bool = True
    ) -> dict[str, tuple[uuid.UUID | None, uuid.UUID | None]]:
    ...
    @property
    def eq_mat(self):
        return self.active_model.eq_mat if isinstance(self.active_model, Model) else self._eq_mat
    ...
    @property
    def nComp(self):
        return self.active_model.nComp if isinstance(self.active_model, Model) else self._nComp
    ...
    @property
    def nStep(self):
        return self.active_model.nStep if isinstance(self.active_model, Model) else self._nStep
    ...
    @property
    def eq_str(self):
        return self.active_model.eq_str if isinstance(self.active_model, Model) else self._eq_str
    ...
    @property
    def eq_mat_str(self):
        return self.active_model.eq_mat_str if isinstance(self.active_model, Model) else self._eq_mat_str
    ...
    @property
    def components(self):
        return self.active_model.components if isinstance(self.active_model, Model) else self._components
    ...
    @property
    def species(self):
        return self.active_model.species if isinstance(self.active_model, Model) else self._species
    ...
    @property
    def comp_concs(self):
        return self.active_model.component_concs if isinstance(self.active_model, Model) else self._comp_concs
    ...
    def binding_constants(self):
        return self.active_model.binding_constants if isinstance(self.active_model, Model) else self._binding_constants
    ...
    def component_names(self):
        return self.active_model.component_names if isinstance(self.active_model, Model) else self._component_names
    ...
    def add_expt_data(self, expt_data: ExptData, emit_events: bool = True) -> None:
        ...
        self.active_expt_data.col_details = {
            k: self.active_expt_data.col_details[k] if k in self.active_expt_data.col_details else {"depindep": None}
            for k in self.active_expt_data.data.columns
        }
    ...
    def new_model(self, name="New model") -> uuid.UUID:
        ...
        if name in existing_names:
            n_models_with_name = len([model for model in self.models.values() if model.name.startswith(name)])
    ...
    async def open_project(self):
        ...
        try:
            with ui.dialog() as dialog, ui.card():
                ...
                upload_box = ui.upload(label="Choose file", auto_upload=True).props('accept=".json, .json.gz"')
    ...
    def to_dict(self):
        ...
        return {
            ...
            "eq_mat_str": (self.eq_mat_str if hasattr(self, "eq_mat_str") else ""),  # Optional attribute
            "components": (
                [asdict(comp) for comp in self.components] if hasattr(self, "components") else []
            ),  # Convert Component objects to dicts
            "species": (self.species if hasattr(self, "species") else []),  # Optional attribute
            # "eq_consts": (
            ...
            "binding_constants": (
                [asdict(k) for k in self.binding_constants] if hasattr(self, "binding_constants") else []
            ),
            "component_names": (self.component_names if hasattr(self, "component_names") else []),  # Optional attribute
            "models": (
                [model.to_dict() for model in self.models.values()] if hasattr(self, "models") else []
            ),  # Optional attribute
            "simulations": (
                [sim.to_dict() for sim in self.simulations.values()] if hasattr(self, "simulations") else []
            ),  # List of simulations
            "raw_datas": (
                [raw_data.to_dict() for raw_data in self.raw_datas.values()] if hasattr(self, "raw_datas") else []
            ),  # List of raw data
            "expt_datas": (
                [expt_data.to_dict() for expt_data in self.expt_datas.values()] if hasattr(self, "expt_datas") else []
            ...
            "fits": ([fit.to_dict() for fit in self.fits.values()] if hasattr(self, "fits") else []),  # List of fits
            "mcmcs": (
                [
                    {
                        "id": str(m.id),
            ...
            "active_raw_data_id": (str(self.active_raw_data_id) if hasattr(self, "_active_raw_data_id") else None),
            "active_mcmc_id": (str(self.active_mcmc_id) if hasattr(self, "_active_mcmc_id") else None),
            "default_model_ids": (
                [str(m) for m in self.default_model_ids] if hasattr(self, "default_model_ids") else []
            ),
        }
    ...
    def from_json(self, json_str: str) -> None:
        ...
        if mtemp:
            for model in mtemp:
                ...
                self.models[m_id].components = [Component(**comp) for comp in model.get("components", [])]
                self.models[m_id].binding_constants = [BindingConstant(**k) for k in model.get("binding_constants", [])]
                self.models[m_id].eq_mat = np.array(model.get("eq_mat", []))  # Convert numpy arrays to lists
                self.models[m_id].component_concs = pd.DataFrame(model.get("component_concs", {}))
        ...
        if rawtemp:
            for raw in rawtemp:
                ...
                raw_data.data = pd.DataFrame(raw.get("data", {}))
        ...
        self._components = [Component(**comp) for comp in comp_temp] if comp_temp else []
    ...
    async def parse_equations(self, e):
        ...
        if self.model_name in [m.name for m in self.models.values()]:
            ...
            # otherwise, we need to ask the user if they want to overwrite the model
                ...
                if res is False:  # i.e. user does not want to save
                    ui.notify("Please give a unique model name and re-try.", type="info")
                    return
                else:
        ...
        if not eq_input:
            ...
            return
        try:
            eq_matrix, component_names, species = eq_mat_from_equation_str_infer_components(eq_input)
            model = self.active_model
            ...
            self.active_model.component_concs = (
                model.component_concs
            )  # Copy the component concentrations from the old model
            ...
            self.components = sorted(
                self.components,
                key=lambda x: component_names.index(x.name) if x.name in component_names else float("inf"),
            )
    ...
    def generate_binding_constants(self):
        ...
        for s in species:
            if s not in [k.species for k in self.active_model.binding_constants if k.species not in species]:
                logK = None
                isComp = False
                vary = True
    ...
    def delete_model(
        self, model: Model, notify_user: bool = True, notify_listeners: bool = True, reconcile: bool = True
    ):
    ...
    def delete_fit(
        self, fit: FitResult, notify_user: bool = True, notify_listeners: bool = True, reconcile: bool = True
    ):
    ...
    def delete_simulation(
        self, simulation: Simulation, notify_user: bool = True, notify_listeners: bool = True, reconcile: bool = True
    ):
    ...
    def delete_expt_data(
        self, expt_data: ExptData, notify_user: bool = True, notify_listeners: bool = True, reconcile: bool = True
    ):
    ...
    def delete_raw_data(
        self, raw_data: RawData, notify_user: bool = True, notify_listeners: bool = True, reconcile: bool = True
    ):
    ...
    def delete_mcmc(
        self, mcmc: MCMCSim, notify_user: bool = True, notify_listeners: bool = True, reconcile: bool = True
    ):
...
bd.makeFitResidPlot(fit,plotMask=(0,1),ylabel='Chemical shift (ppm)')"""
    ...
    def generate_binding_model_for_fit(
        ...
        obs_list = [
            x
            for x in self.active_expt_data.sorted_data.columns
            if self.active_expt_data.col_details[x]["depindep"] == "dep"
        ]
        ...
        if self.active_expt_data.has_linear_obs(self._expt_dtypes):
            ...
            if (
                abs_to_spec is not None
                and isinstance(abs_to_spec, np.ndarray)
                and abs_to_spec.ndim == 2
                and abs_to_spec.size > 0
            ):
        ...
        for k in self.active_model.binding_constants:
            model.params[f"log{k.species}"].set(
                value=k.logK if k.logK is not None else 0.0, vary=k.vary, min=k.min, max=k.max
            )
    ...
    def dump_fit_notebook(self, fit: FitResult) -> tuple[dict, "pd.DataFrame"]:
        ...
        if expt_data is not None:
            obs_type_names: list[str] = [ot.name for ot in expt_data.get_obs_list(self._expt_dtypes)]
