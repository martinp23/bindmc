...
def eqMatFromEqnStr(eq_str: str, components: list) -> tuple[np.ndarray, list, list]:
    ...
    for ii, lhs in enumerate(lhs):
        ...
        if sorted(lhscomp) != sorted(rhscomp):
            raise ValueError(
                f"Left-hand side and right-hand side of reaction {ii + 1} do not match: {lhscomp} != {rhscomp}"
            )
    ...
    allspecies_list = [comp for comp in components if comp in allspecies_list] + [
        spec for spec in allspecies_list if spec not in components
    ]
