...
class BayesPriorEditor:
    ...
    async def open(self) -> None:
        ...
        if self.priors:
            prior_by_label = {str(prior.get("label", "")): prior for prior in self.priors if isinstance(prior, dict)}
        ...
        with (
            ui.dialog() as dialog,
            ui.card().classes("w-[min(980px,96vw)] max-h-[88vh] overflow-hidden bayes-priors-card"),
        ):
            ui.label("Edit MCMC Priors").classes("text-lg font-bold")
            ui.label("Uniform priors use lower/upper bounds. None keeps the current model bounds.").classes(
                    ...
                    with ui.card().classes("w-full mb-2"):
                        with ui.row().classes("w-full items-center gap-3 flex-wrap"):
                            ...
                            type_select = ui.select(
                                ...
                                value="Uniform"
                                if prior is None
                                else ("None" if str(prior.get("type", "uniform")).lower() == "none" else "Uniform"),
                                label="Prior type",
                            ).classes("w-40")
                            ...
                            async def _open_apply_modal(source_row=row_state) -> None:
                                ...
                                same_type_others = [
                                    r for r in row_states if r is not source_row and r["param_type"] == pt
                                ]
            ...
            async def _save_priors() -> None:
                ...
                for row_state in row_states:
                    ...
                    if lower_value is None or upper_value is None:
                        status_label.text = (
                            f"{row_state['label']}: lower and upper bounds are required for a uniform prior."
                        )
                        return
                    try:
                    ...
                    if lower_float > upper_float:
                        status_label.text = (
                            f"{row_state['label']}: lower bound must be less than or equal to upper bound."
                        )
                        return
